...
function. These plots correspond to the Figure 2 in the manuscript. 
...
ggarrange(plot.Tx_C2P1T04+ rremove("x.text") + rremove(object = "xlab") , plot.Vx_C2P1T04, 
...
ggarrange(plot.Tx_T1P1T06+ rremove("x.text") + rremove(object = "xlab") , plot.Vx_T1P1T06, 
         ncol = 1, nrow = 2, align = "v")
...
We calculate the iterative distance covered by the water and food tracers 
...
We calculate the total distance covered by each tracer over the entire sequence,
by summing the previously computed iterative distances. We name this 
...
tracers of the trial. As there was one to three markers implanted in one food 
...
For example, **Carps_food** is a list with the mean and standard deviation of 
the cumulative distance of each trial for the food item
...
The food tracers don't have any standard deviation bars because all the markers
were on the same food item.
...
# compute the differences at each row of Mean_v_t_W to obtain a trend (+ or -) 
...
# compute a label (stasis, reverse flow, transport) associated with a given 
...
We test the phase detection on the different trials. Here we chose "C2P1T04".
...
We use the minimum and maximum of the **average_VX** data to obtain the maximum amplitude 
of the water tracers motion during the first strike. The X axis is pointing 
...
# Compute the minimum of average_vX during the first strike
...
# Compute the maximum of average_vX during the entire sequence
...
# Compute the anteroposterior velocity at the beginning of the sequence in a given species for the water (W) and the food (F):
...
# Compute the peak of anteroposterior velocity during the first strike in a given species :
...
# Compute the peak of anteroposterior velocity during the rest of the sequence in a given species :
...
components contribution for each phase of the suction feeding sequence among 
the 13 analyzed trials for each item (water and food). 

### First strike

We extract the first strike data, based on the "phase" variable. 

```{r FirstStrike_comp, echo=TRUE, message=FALSE, warning=FALSE, fig.width=7, fig.show='hold',fig.align='center'}
...
in comparison to the food tracer trajectory.  
  - Calculating the angle formed by the position of a given particle during 
  three consecutive frame  
  - Calculating the curvature formed by the position of a given particle during
  three consecutive frame  
...
t_test_p_value_Carps <-map(.x = Intervals, 
                           .f = ~get_sublist(lst = t_test_results_Carps[[.]], 
                                             group_name = "p.value")) %>%
  set_names(Intervals)
...
t_test_p_value_Tilapias <-map(.x = Intervals, 
                           .f = ~get_sublist(lst = t_test_results_Tilapias[[.]],
                                             group_name = "p.value")) %>%
  set_names(Intervals)
...
##### kruskal.test
We perform an anova on each interval.
...
kruskal.test_p_value_Carps <-map(.x = Intervals, 
                           .f = ~get_sublist(lst = t_test_results_Carps[[.]], 
                                             group_name = "p.value")) %>%
  set_names(Intervals)
...
kruskal.test_p_value_Tilapias <-map(.x = Intervals, 
                           .f = ~get_sublist(lst = t_test_results_Tilapias[[.]],
                                             group_name = "p.value")) %>%
  set_names(Intervals)
...
d <- cbind(rbind(Mean_tx_W[[trial]][subtime,],Mean_tx_F[[trial]][subtime,]), 
           time = Timing[[trial]]$time[subtime])

plot.Tx_C2P1T04 <- plot_mean(d, ylab=ylab, xlab=xlab, subtitle = subtitle, 
...
plot.Tx_T1P1T06 <- plot_mean(d, ylab=ylab, xlab=xlab, subtitle = subtitle, 
                             color = color)  + theme(plot.margin = unit(c(0,0,0,0), "lines")) + theme_minimal()+
  theme(legend.position = "none", panel.grid.minor = element_blank()) 
