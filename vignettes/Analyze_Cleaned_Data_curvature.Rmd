...
plot.Vx_T1P1T06 <- plot_mean(d, ylab=ylab, xlab=xlab, 
                             color = color, subtitle = subtitle)
...
We use the minimum of the **average_VX** data to obtain the maximum amplitude 
the water tracers motion during the first strike as the X axis is pointing 
...
- For the carps, the maximum amplitude during the first strike is 
`r round(mean(Vx_FS_Carps), digits = 2) ` $\pm$ 
`r round(x=sd(Vx_FS_Carps), digits=2) ` $cm/s$.  
- For the tilapias, the maximum amplitude during the first strike is `r round(mean(Vx_FS_Tilapias), digits = 2) ` 
$\pm$ `r round(x=sd(Vx_FS_Tilapias), digits=2) ` $cm/s$.
  
The high standard deviation for the carps is due to trial C1P2T13, with a huge 
amplitude compared to the other trials.
...
for(trial in Trials){
  ...
  # we start the analysis when the particles passed the entrance of the mouth 
  #(different between specimens) 
  ...
  # We want to clean the particles that circle back towards the entrance of 
  #the mouth (using lag)
  
  Particles <- levels(factor(data_cut$Grp)) # all particles in a given trial 
  
  Curling <- list()
  data_cut_clean <- 0
  for (i in 1:length(Particles)){
    A <- data_cut[which(data_cut$Grp == Particles[i]),]
    Curling[[i]] <- which(diff(x = A$WF_tx, lag = 1)>0)[1] #beginning of curling
...
# We want to clean the particles that circle back towards the entrance of 
# the mouth (using lag)
Particles <- levels(factor(data_cut_example$Grp)) 
Curling <- list()
data_cut_clean_example <- 0
for (i in 1:length(Particles)){
  A <- data_cut_example[which(data_cut_example$Grp == Particles[i]),]
  Curling[[i]] <- which(diff(x = A$WF_tx, lag = 1) >0)[1]  
  if (is.na(Curling[[i]]) == TRUE) { Curling[[i]] = nrow(A) }
...
data_cut_clean_Water_example <- data_cut_clean_example %>% 
  filter(item == "W")

data_cut_clean_Food_example <- data_cut_clean_example %>% 
  filter(Grp == "A01_data.tx_C2P1T04")
...
Divergence[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_tz")]][j] <- 
      ...
      A[2] <- 0 #delete y coordinate (mediolateral motions)
      B[2] <- 0
      C[2] <- 0
...
# # plot the trajectories  #tag:: can be deleted start
# 
# Divergence_df %>% filter(item == "W") %>% 
#   filter(tx_pcent > 1 & tx_pcent < 80) %>% 
#   filter(str_detect(string = particle, pattern = "C")) %>% 
#   ggplot(aes(y=tz, x=tx_pcent)) +
#   geom_line(aes(group=particle, color= item), size = 0.8) +
#   geom_line(data = Divergence_df %>% filter(item == "F") %>% 
#               filter(tx_pcent > 1 & tx_pcent < 80), 
...
# 
# Divergence_df %>% filter(item == "W") %>% 
#   filter(tx_pcent > 20 & tx_pcent < 80) %>% 
...
<!-- Divergence_df %>% filter(item == "W") %>% -->
<!--   filter(tx_pcent > 1 & tx_pcent < 95) %>%  -->
<!-- ggplot(aes(y=curvature, x=tx_pcent)) + -->
<!--  geom_line(aes(group=particle, color= item), size = 0.8) + -->
<!--  geom_line(data = Divergence_df %>% filter(item == "F") %>%  -->
<!--              filter(tx_pcent > 1 & tx_pcent < 100),  -->
<!--            aes(group=particle, color= item), size = 0.8) + -->
<!--     ylab("curvature (m-1)") + -->
<!--     xlab("tx (cm)")+ -->
...
for(trial in Trials){
  ...
  Divergence[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_tz")]][j] <- 
    NA
      ...
      # the lateral view (dorso-ventral motins)
      B[3] <- 0
      C[3] <- 0
...
ggplot(aes(y=-angle, x=tx_pcent)) +
 ...
 geom_path(data = Divergence_df %>% filter(item == "F") %>%
             filter(tx_pcent > 1 & tx_pcent < 75),
           aes(group=particle, color= item), size = 0.8) +
    ylab("angle (deg)") +
    xlab("tx (cm)")+
...
KData_Gape <-  ggplot(KData_df, aes(x = Time, y = Gape, colour="#808080")) +
...
KData_Hyoid <-  ggplot(KData_df,
                       aes(x = Time,
                           y = -BonesLocators_relative.hyoid_tip_data.ty,
                           colour="#808080")) + 
  geom_line(aes(group=1), size=size)+
  ylab("") + 
...
KData_Ope <-  ggplot(KData_df, 
                     aes(x = Time, y = BonesLocators_relative.ope_tip_data.tz,
...
KData_Gape <-  ggplot(KData_df,
                      aes(x = Time, y = Gape, colour="#808080")) +
  geom_line(aes(group=1), size=size)+
  ylab("") + 
  labs(subtitle = "Gape (cm)")+
  theme_minimal()+
...
## Ope
KData_Ope <-  ggplot(KData_df, aes(x = Time, 
                                   y = -BonesLocators_relative.ope_tip_data.tz,
                                   colour="#808080")) +   
  geom_line(aes(group=1), size=size)+
   ylab("") + 
  labs(subtitle = "Opercula abduction (cm)")+
