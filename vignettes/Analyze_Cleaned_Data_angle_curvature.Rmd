...
<!--       Angle[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_angle")]][j] <- as.vector(angle3D(A,B,C)) -->
<!--       Angle[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_tx_pcent")]][j] <- unlist(Temp_particle $ WF_tx_pcent[j]) -->
<!--       Angle[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_tx")]][j] <- unlist(Temp_particle[j,1]) -->
<!--       Angle[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_ty")]][j] <- unlist(Temp_particle[j,2]) -->
<!--       Angle[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_tz")]][j] <- unlist(Temp_particle[j,3]) -->
<!--     } -->
<!--   } -->
<!-- } -->
...
<!-- We need to transform the Angle list into a data.frame with the particle name, angle, tx, tx_pcent, and tz.   -->
...
<!-- Angle_df_angle <- map(.x = str_which(string = names(Angle_df_list), pattern = "angle"), .f = ~get_sublist(lst = Angle_df_list, group_name = names(Angle_df_list)[[.]])) -->

<!-- Angle_df_tx_pcent <- map(.x = str_which(string = names(Angle_df_list), pattern = "_tx_pcent"), .f = ~get_sublist(lst = Angle_df_list, group_name = names(Angle_df_list)[[.]])) -->

<!-- Angle_df_tz <- map(.x = str_which(string = names(Angle_df_list), pattern = "_tz"), .f = ~get_sublist(lst = Angle_df_list, group_name = names(Angle_df_list)[[.]])) -->
...
<!-- Angle_df <- Angle_df %>% mutate(item = case_when(str_detect(string = particle, pattern = "F") ~ "W",  -->
...
<!--  geom_path(data = Angle_df %>% filter(item == "F") %>% filter(tx_pcent > 2 & tx_pcent < 82), aes(group=particle, color= item), size = 0.8) + -->
<!--     ylab("tz (cm)") + -->
<!--     xlab("tx (cm)")+ -->
<!--     scale_color_manual(values = color)+ -->
<!--       theme_minimal()  -->
...
<!-- Angle_df_water <-  Angle_df %>% filter(item == "W") %>% filter(tx_pcent > 2 & tx_pcent < 75)%>% arrange(desc(tx_pcent)) -->
<!-- Angle_df_food <-  Angle_df %>% filter(item == "F") %>% filter(tx_pcent > 2 & tx_pcent < 75)%>% arrange(desc(tx_pcent)) %>% filter(str_detect(string = particle, pattern = "T2P1T10", negate = TRUE)) -->
...
<!-- Angle_interval_water_mean <-  Angle_interval_water %>%   summarise( mean.angle = mean(angle, na.rm=TRUE), sd.angle = sd(angle, na.rm=TRUE) ) -->
...
<!--  Angle_interval_food_mean <- Angle_interval_food %>% summarise( mean.angle = mean(angle, na.rm=TRUE) , sd.angle = sd(angle, na.rm=TRUE) ) -->
...
<!--  # # organized by trial  -->
<!--  #  Angle_interval_trial <- map(.x = Trials, .f = ~(filter(.data = Angle_interval, str_detect(string = particle, pattern = names(Maya_WF[.]))))) %>% set_names(names(Maya_WF[Trials])) -->
...
<!--  Angle_interval_species <- map(.x = Species, .f = ~(filter(.data = Angle_interval, str_detect(string =  str_sub(string = particle, start = 4, end = 7 ), pattern = .)))) %>% set_names(Species) -->
...
<!-- # Carps -->
<!-- species <- "C" -->
<!-- Intervals <-levels(Angle_interval_species[[species]]$interval) -->
<!-- Intervals <- Intervals[1:4] -->
<!-- t_test_results_Carps <- map(.x = Intervals,  -->
<!--                             .f = ~(t.test(angle ~ item, -->
<!--                                           data = filter(.data = Angle_interval_species[[species]], interval== .), -->
...
- we calculate the angle made by the trajectory of each particle in the medio-lateral angle.
...
for(trial in Trials){
  ...
  for (i in 1:length(Particles)){
    ...
    if (nrow(Temp_particle) <= 1) {
      ...
      Divergence[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_angle")]][j] <- as.vector(angle3D_curv(A,B,C)[1])
      Divergence[[names(Maya_WF)[trial]]][[name]][[paste0(name, "_curvature")]][j] <- as.vector(angle3D_curv(A,B,C)[2])
...
Divergence_df_list <- unlist(unlist(Divergence, recursive = FALSE), recursive = FALSE)
...
Divergence_df %>% filter(item == "W") %>% filter(tx_pcent > 1 & tx_pcent < 100) %>% 
ggplot(aes(y=-angle, x=tx_pcent)) +
 geom_path(aes(group=particle, color= item), size = 0.8) +
 geom_path(data = Divergence_df %>% filter(item == "F") %>% filter(tx_pcent > 1 & tx_pcent < 100), aes(group=particle, color= item), size = 0.8) +
    ylab("angle (deg)") +
    xlab("tx (cm)")+
...
Divergence_df %>% filter(item == "W") %>% filter(tx_pcent > 1 & tx_pcent < 74) %>% 
ggplot(aes(y=curvature, x=tx_pcent)) +
 geom_path(aes(group=particle, color= item), size = 0.8) +
 geom_path(data = Divergence_df %>% filter(item == "F") %>% filter(tx_pcent > 1 & tx_pcent < 74), aes(group=particle, color= item), size = 0.8) +
    ylab("tz (cm)") +
    xlab("tx (cm)")+
...
# split between water and food particles
Divergence_df_water <-  Divergence_df %>% filter(item == "W") %>% filter(tx_pcent > 1 & tx_pcent < 85)%>% arrange(desc(tx_pcent))
Divergence_df_food <-  Divergence_df %>% filter(item == "F") %>% filter(tx_pcent > 1 & tx_pcent < 85)%>% arrange(desc(tx_pcent)) %>% filter(str_detect(string = particle, pattern = "T2P1T10", negate = TRUE))
...
# water -----------------
Angle_interval_water <- Divergence_df_water %>% 
    mutate(interval = cut(tx_pcent,
                          breaks, 
                          include.lowest = TRUE, 
                          right = FALSE)) %>% group_by(interval)
Angle_interval_water_mean <-  Angle_interval_water %>%   summarise( mean.angle = mean(angle, na.rm=TRUE), sd.angle = sd(angle, na.rm=TRUE) )
 ...
 # food -----------------
Angle_interval_food <- Divergence_df_food %>% 
    mutate(interval = cut(tx_pcent,
                          breaks, 
                          include.lowest = TRUE, 
                          right = FALSE)) %>% group_by(interval) 
...
Curvature_interval_species <- map(.x = Species, .f = ~(filter(.data = Curvature_interval, str_detect(string =  str_sub(string = particle, start = 4, end = 7 ), pattern = .)))) %>% set_names(Species)
  ...
  ggplot(Curvature_interval_species[[species]], aes(x=interval, y=curvature, fill=item)) +
    ...
    theme_minimal() +
    stat_summary(fun=mean, geom="point", size=1, col = "white", position = position_dodge(0.75)) +
    theme(legend.position = "none")+   ylim(-30,30) 
